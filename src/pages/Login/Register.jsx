...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      setError("Bạn phải ít nhất 14 tuổi để đăng ký");
      return;
    }
    ...
    try {
      ...
      if (response) {
        // Đăng ký thành công
        // Nếu có ảnh đại diện, gọi updateAvatar
        if (profilePicFile) {
          try {
            await authService.updateAvatar(profilePicFile);
          } catch (avatarError) {
            console.error("Avatar update error:", avatarError);
            setError("Đăng ký thành công nhưng cập nhật ảnh đại diện thất bại");
          }
        }
        navigate("/login", {
